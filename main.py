...
from creepypasta_dataset import *
from lstm_model import *
from train import *

def main():
    input_file_path = 'all_creepypasta_bodies.txt'
    ...
    train_dataset = CreepypastaDataset(X_train, y_train)
    train_dataloader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True)

    val_dataset = CreepypastaDataset(X_val, y_val)
    val_dataloader = DataLoader(val_dataset, batch_size=config['batch_size'], shuffle=False)

    test_dataset = CreepypastaDataset(X_test, y_test)
    ...
    model = LSTMModel(vocab_size=vocab_size, embed_size=config['embed_size'], hidden_size=config['hidden_size'], num_layers=config['num_layers']).to(device)

    train(model=model, device=device, train_dataloader=train_dataloader, val_dataloader=val_dataloader, config=config)
